...
// import { ViewMycalendarDialog } from "./view-mycalendar-dialog"
import { Dialog, DialogTrigger } from "@radix-ui/react-dialog"
import useMyCalendarStore from "../store/useMyCalendarStore"
...
export function MyCalendar() {
    ...
    const getDateColor = (datetimeStart) => {
    ...
    return (
        <div>
            {/* citas de hoy */}
            <div className="flex flex-1 flex-col gap-4 p-4">
                <h1 className="text-3xl font-bold text-center text-pink-700 mb-6">Citas para Hoy</h1>
                <div className="flex flex-wrap justify-center gap-6">
                {myDates.map((date) => (
                    isSameDay(new Date(date.datetimeStart), new Date()) && (
                        <Dialog key={date.id} open={isDialogOpen && myDate.id === date.id} onOpenChange={setIsDialogOpen}>
                            <DialogTrigger asChild>
                                <div
                                    className={`p-6 rounded-2xl shadow-lg cursor-pointer transition flex flex-col ${getDateColor(date.state)}`}
                            ...
                            {myDate && myDate.id === date.id && (
                                <ViewMycalendarDialog appointment={myDate} />
                            )}
                        </Dialog>
                    )
                ))}
            ...
            {/* todas las citas */}
            <div className="flex flex-1 flex-col gap-4 p-4">
                <h1 className="text-3xl font-bold text-center text-pink-700 mb-6">Mis Citas en la Agenda</h1>
                <div className="flex flex-wrap justify-center gap-6">
                    {myDates.map((date) => (
                        <Dialog key={date.id} open={isDialogOpen && myDate.id === date.id} onOpenChange={setIsDialogOpen}>
                            <DialogTrigger asChild>
