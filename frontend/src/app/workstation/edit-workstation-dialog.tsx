...
import useWorkstationStore, { type Workstation } from "../store/useWorkstationStore"
import useCategoryStore from "../store/useCategoryStore"
import { z } from "zod"
import { useForm } from "react-hook-form"
import { zodResolver } from "@hookform/resolvers/zod"
import { Form, FormControl, FormDescription, FormField, FormItem, FormLabel, FormMessage } from "@/components/ui/form"
import { Loader2, Pencil, Monitor } from "lucide-react"
...
export function EditWorkstationDialog({ workstation }: { workstation: Workstation }) {
  const [isOpen, setIsOpen] = useState(false)
  const [isSubmitting, setIsSubmitting] = useState(false)

  const updateWorkstation = useWorkstationStore((state) => state.updateWorkstation)
  ...
  const form = useForm<WorkstationFormValues>({
    ...
    defaultValues: {
      ...
      categories: workstation.categories?.map((cat) => cat.id) || [],
    },
  })
  ...
  const onSubmit = async (data: WorkstationFormValues) => {
    ...
    try {
      ...
      const success = await updateWorkstation(workstationData)
      if (success) {
        setIsOpen(false)
      }
    } catch (error) {
