...
import usePackageStore, { type Package } from "../store/usePackageStore"
import useServiceStore from "../store/useServiceStore"
import { z } from "zod"
import { useForm } from "react-hook-form"
import { zodResolver } from "@hookform/resolvers/zod"
import { Form, FormControl, FormDescription, FormField, FormItem, FormLabel, FormMessage } from "@/components/ui/form"
import { Loader2, PackageIcon, Pencil } from "lucide-react"
...
export function EditPackageDialog({ pkg }: { pkg: Package }) {
  ...
  const [totalDuration, setTotalDuration] = useState(pkg.duration)
  ...
  // Inicializar los servicios seleccionados
  useEffect(() => {
    if (pkg.services && Array.isArray(pkg.services)) {
      const serviceIds = pkg.services.map((service) => service.id.toString())
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" size="icon" className="h-8 w-8 border-purple-200 text-purple-700 hover:bg-purple-50">
                  ...
                  <FormItem>
                    ...
                    {servicesLoading ? (
                      ...
                      <div className="p-4 text-center border rounded-md border-purple-200">
                        ...
                        <p className="text-sm text-gray-400 mt-1">Debe crear servicios antes de editar paquetes</p>
                      </div>
                    ) : (
                      <>
